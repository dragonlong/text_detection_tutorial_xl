...
from lstm.modelrnn import VectModel
from lstm.input_node import DetectorInput
...
#             model.import_ops(config)
        ...
        with sv.managed_session(config=config_proto) as session:
            ...
            for i in range(config.max_max_epoch):
                ...
                train_perplexity, predicts_tr = run_epoch(session, m, i, eval_op = m.train_op, summary_writer= train_writer, verbose = True)
                print("Epoch: %d training Perplexity: %.3f" % (i+1, train_perplexity))
                valid_perplexity, predicts_val = run_epoch(session, mvalid, i, summary_writer= val_writer)
                print("Epoch: %d Valid Perplexity: %.3f" % (i+1, valid_perplexity))
                test_perplexity,  predicts_test = run_epoch(session, mtest, i, summary_writer= test_writer)
